...
function Adduser()
{
    const [formValue, setFormValue]= useState({ email:'',phone:'',address:''});
    const [message, setMessage]= useState();
    const navigate= useNavigate();
    const handleInput=(e)=>{
     const {name, value}= e.target;
    ...
    const handleSubmit= async(e)=>{
       e.preventDefault();
       const allInputvalue= { email:formValue.email, mobile:formValue.phone, message:formValue.address}; 
       console.log('first',allInputvalue)

      let res= await fetch(`https://3.111.17.212:3001/api/v1/contactus/addcontacts`,{
        method:"POST",
        headers:{'content-type':'application/json'},
        body:JSON.stringify(allInputvalue)
      });
